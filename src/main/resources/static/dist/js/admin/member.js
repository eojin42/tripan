...
function openDetailModal(event, btnElement) {
  ...
  if (!memberId) { alert('회원 식별자를 찾을 수 없습니다.'); return; }
...
function openInfoModal(memberId) {
  ...
  fetch(contextPath + '/admin/member/detail/' + memberId)
    ...
    .then(data => {
      ...
      document.getElementById('infoLastLogin').innerText   = data.lastLoginAt
        ? new Date(data.lastLoginAt).toLocaleString('ko-KR') : '-';
      document.getElementById('infoModal').style.display = 'flex';
    })
    .catch(err => { console.error(err); alert('회원 정보를 불러오는데 실패했습니다.'); });
}
...
function updateKPI(oldStatus, newStatus) {
  ...
  if (oldStatus === '1') activeCnt--; else if (oldStatus === '2') banCnt--;
  if (newStatus === '1') activeCnt++; else if (newStatus === '2') banCnt++;
...
function copySelectedIds() {
  ...
  }).catch(() => { prompt('아래 ID를 복사하세요:', ids.join(', ')); });
}
...
function applyBulkStatus() {
  ...
  document.querySelectorAll('.member-row').forEach(row => {
    ...
    if (row.style.display !== 'none' && cb.checked) {
      targetIds.push(parseInt(cb.dataset.memberid));
    }
  });
  if (targetIds.length === 0) { alert('선택된 회원이 없습니다.'); return; }

  // 서버 호출
  fetch(contextPath + '/admin/member/status/bulk', {
    method: 'POST',
  ...
  .then(() => {
    const statusUI = {
      '1': '<span class="badge badge-done">정상</span>',
      '2': '<span class="badge badge-danger">BAN(정지)</span>',
      '3': '<span class="badge" style="background:#fef3c7;color:#b45309;">휴면</span>',
      '4': '<span class="badge" style="background:var(--bg);color:var(--muted)">탈퇴 완료</span>'
    };
    const changedRows = [];
    document.querySelectorAll('.member-row').forEach(row => {
      const cb = row.querySelector('.row-check');
      if (row.style.display !== 'none' && cb.checked) {
        ...
        if (statusCell) statusCell.innerHTML = statusUI[newStatus] || '';
        cb.checked = false;
    ...
    alert('✅ ' + changedRows.length + '명의 상태가 변경되었습니다.');
  })
  .catch(err => {
    console.error(err);
