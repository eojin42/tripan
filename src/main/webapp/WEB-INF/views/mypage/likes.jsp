...
<head>
  ...
  <style>
    ...
    /* 좋아요 그리드 */
    .like-grid { display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px; }
    .like-card {
    ...
    .like-card:hover { transform:translateY(-4px);box-shadow:0 12px 28px rgba(137,207,240,.15);border-color:rgba(137,207,240,.4); }
    .like-thumb { width:100%;height:130px;object-fit:cover;background:#F0F8FF; }
    .like-thumb-placeholder { width:100%;height:130px;background:linear-gradient(135deg,#E6F4FF,#FFE4E8);display:flex;align-items:center;justify-content:center;color:var(--sky-blue);font-size:28px; }
    .like-body { padding:14px; }
    .like-name { font-size:14px;font-weight:800;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }
    .like-meta { font-size:11px;color:var(--text-gray);font-weight:600;display:flex;align-items:center;gap:4px; }
    .like-meta i { color:var(--sky-blue); }
    .like-badge { position:absolute;top:10px;left:10px;background:rgba(255,255,255,.92);backdrop-filter:blur(4px);border-radius:8px;padding:3px 8px;font-size:10px;font-weight:800;color:var(--text-dark); }
    .btn-unlike {
    ...
    .btn-unlike:hover { background:#FF6B6B;color:white;transform:scale(1.1); }

    /* 게시글 리스트 (피드/자유게시판) */
    .post-list { display:flex;flex-direction:column;gap:12px; }
    .post-card {
      background:white;border-radius:16px;padding:18px 20px;
      border:1px solid var(--border-light);box-shadow:0 2px 8px rgba(0,0,0,.02);
      transition:.2s;cursor:pointer;display:flex;align-items:center;justify-content:space-between;gap:16px;position:relative;
    }
    .post-card:hover { box-shadow:0 6px 20px rgba(137,207,240,.12);border-color:rgba(137,207,240,.3);transform:translateX(4px); }
    .post-info { flex:1;min-width:0; }
    .post-title { font-size:14px;font-weight:800;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }
    .post-meta { font-size:11px;color:var(--text-gray);font-weight:600;display:flex;align-items:center;gap:8px; }
    .post-meta i { color:var(--light-pink); }
    .post-thumb { width:60px;height:60px;border-radius:12px;object-fit:cover;flex-shrink:0; }
    .post-type-tag { font-size:10px;font-weight:800;padding:3px 8px;border-radius:8px;background:var(--sky-blue-light);color:var(--sky-blue);flex-shrink:0; }
    ...
    @media(max-width:600px){ .like-grid { grid-template-columns:repeat(2,1fr); } }
  </style>
</head>
<body>
...
<main class="mypage-container">
  <jsp:include page="/WEB-INF/views/layout/mypage_sidebar.jsp">
    <jsp:param name="activeMenu" value="likes"/>
  </jsp:include>

  <div class="content-area">
    <div>
      <h2 class="page-title">나의 좋아요 목록</h2>
      <p class="page-subtitle">❤️ 좋아요를 누른 피드, 게시글을 모아볼 수 있어요</p>
    </div>

    <div class="tab-bar">
      <button class="tab-btn active" onclick="switchTab('feed', this)"><i class="bi bi-image"></i> 피드</button>
      <button class="tab-btn" onclick="switchTab('freeboard', this)"><i class="bi bi-chat-square-text"></i> 자유게시판</button>
    </div>

    <div id="tab-feed"><div id="feed-area"><div class="spin"></div></div></div>
    <div id="tab-freeboard" style="display:none;"><div id="freeboard-area"><div class="spin"></div></div></div>
  </div>
</main>
...
<script>
  ...
  function switchTab(tab, btn) {
    ...
    ['feed','freeboard'].forEach(function(t) {
      document.getElementById('tab-' + t).style.display = t === tab ? '' : 'none';
    });
    if (!loaded[tab]) {
      loaded[tab] = true;
      if (tab === 'feed')       loadPostLikes('FEED');
      if (tab === 'freeboard')  loadPostLikes('FREEBOARD');
    }
  }

  // ── 피드 / 자유게시판 좋아요 ──
  async function loadPostLikes(type) {
    var areaId = type === 'FEED' ? 'feed-area' : 'freeboard-area';
    var area = document.getElementById(areaId);
    var emptyIcon = type === 'FEED' ? 'bi-image' : 'bi-chat-square-text';
    var emptyMsg  = type === 'FEED' ? '좋아요한 피드가 없어요' : '좋아요한 게시글이 없어요';
    var listUrl   = type === 'FEED' ? '${pageContext.request.contextPath}/community/feed' : '${pageContext.request.contextPath}/community/freeboard';

    try {
      var res = await fetch('${pageContext.request.contextPath}/mypage/api/likes?type=' + type);
      if (!res.ok) throw new Error();
      var list = await res.json();
      if (!list || !list.length) {
        area.innerHTML = renderEmpty(emptyIcon, emptyMsg, listUrl);
        return;
      }
      area.innerHTML = '<div class="post-list">' +
        list.map(function(p) {
          var detailUrl = type === 'FEED'
        	    ? '${pageContext.request.contextPath}/community/feed'
        	    : '${pageContext.request.contextPath}/community/feed?tab=freeboard';
        	    return '<div class="post-card" onclick="location.href=\'' + detailUrl + '\'">' +
            '<div class="post-info">' +
              '<div class="post-title">' + escHtml(p.targetName || '(제목 없음)') + '</div>' +
              '<div class="post-meta"><i class="bi bi-heart-fill"></i>' + fmtRelative(p.createdAt) + ' 좋아요</div>' +
            '</div>' +
            '<span class="post-type-tag">' + '</span>' +
            '<button class="btn-unlike" onclick="unlike(event,' + (p.likeId || p.id) + ', this)" title="좋아요 취소"><i class="bi bi-heart-fill"></i></button>' +
          '</div>';
        }).join('') +
      '</div>';
    } catch(e) {
      area.innerHTML = renderEmpty(emptyIcon, '불러오기에 실패했어요', listUrl);
    }
  }

  // ── 좋아요 취소 ──
  async function unlike(e, likeId, btn) {
    e.stopPropagation();
    try {
      var res = await fetch('${pageContext.request.contextPath}/mypage/api/likes/' + likeId, { method: 'DELETE' });
      if (!res.ok) throw new Error();
      var card = btn.closest('.like-card') || btn.closest('.post-card');
      card.style.transition = 'all .3s';
      card.style.opacity = '0';
      card.style.transform = 'scale(0.9)';
      setTimeout(function() { card.remove(); }, 300);
    } catch(e) { alert('좋아요 취소에 실패했어요'); }
  }
