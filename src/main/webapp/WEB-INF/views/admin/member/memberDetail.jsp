...
<head>
  ...
  <style>
    ...
    .modal-sheet { background:#fff; width:100%; max-width:460px; border-radius:20px; box-shadow:0 24px 64px rgba(0,0,0,.18); overflow:hidden; animation:modalUp .35s cubic-bezier(.16,1,.3,1); display:flex; flex-direction:column; }
  ...
  <div class="main-wrapper">
    ...
    <main class="main-content">
      ...
      <div class="card profile-header-card fade-up">
        <img src="${not empty member.profilePhoto ? member.profilePhoto : pageContext.request.contextPath.concat('/dist/images/trip_icon.png')}" 
      ...
      <div class="detail-grid fade-up">
        ...
        <div class="card-pad">
          ...
          <dl class="info-list">
            ...
            <div class="info-row"><dt>포인트 잔액</dt><dd><fmt:formatNumber value="${not empty member.pointBalance ? member.pointBalance : 0}" pattern="#,###"/>P</dd></div>
            <div class="info-row"><dt>보유 쿠폰</dt><dd>${not empty member.couponCount ? member.couponCount : 0}개</dd></div>
            <hr style="border:0;height:1px;background:var(--border);margin:2px 0;">
            <div class="info-row"><dt>신고 횟수</dt>
        ...
        <div class="table-responsive">
          <table>
            ...
            <c:forEach var="book" items="${bookingList}">
              <%-- onclick에 따옴표 충돌 방지: data-* 속성으로 값 전달 --%>
              <tr style="cursor:pointer;"
                ...
                <td>
                  <strong>${book.placeName}</strong><br>
                  <small style="color:var(--muted);font-size:12px;">${book.roomName}</small>
                </td>
      ...
      <div class="receipt-main">
        ...
        <div class="r-section">
          ...
          <div class="price-table">
            ...
            <div class="price-row discount" id="md_couponRow" style="display:none;">
              <span>쿠폰 할인</span><strong id="md_coupon"></strong>
            </div>
            <div class="price-row discount" id="md_pointRow" style="display:none;">
              <span>포인트 사용</span><strong id="md_point"></strong>
            </div>
...
function openResModal(row) {
  ...
  if (couponN > 0) {
    document.getElementById('md_coupon').textContent = '- ₩' + couponN.toLocaleString('ko-KR');
    couponRow.style.display = 'flex';
  } else {
    couponRow.style.display = 'none';
  }
  if (pointN > 0) {
    document.getElementById('md_point').textContent = '- ₩' + pointN.toLocaleString('ko-KR');
  ...
  b.className   = d.status === 'CANCELED' ? 'badge badge-danger'
                : d.status === 'SUCCESS'  ? 'badge badge-done'
                : 'badge badge-wait';
...
function filterReservations() {
  ...
  document.querySelectorAll('#resTableBody tr[data-resdate]').forEach(function(row) {
    ...
    var target  = type === 'name' ? (row.dataset.name || '').toLowerCase()
                : row.querySelector('td') ? row.querySelector('td').textContent.toLowerCase() : '';
...
window.addEventListener('click', function(e) {
  if (e.target.classList.contains('modal-overlay')) {
    closeResModal(); closeSuspendModal();
  }
});
</script>
