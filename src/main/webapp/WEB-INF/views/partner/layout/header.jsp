...
<style>
  ...
  .profile-dropdown.show,
    .property-switcher:hover .profile-dropdown,
  ...
  <div class="header-left" style="display: flex; align-items: center; gap: 16px;">
    <div class="property-switcher" style="position: relative;">
      <button class="property-btn" onclick="document.getElementById('propertyDrop').classList.toggle('show')" 
        ...
        <span id="currentProperty">
            <c:choose>
                <c:when test="${not empty currentPartner.partnerName}">${currentPartner.partnerName}</c:when>
                <c:otherwise>새로운 숙소 등록하기</c:otherwise>
            </c:choose>
        </span> 
      ...
      <div id="propertyDrop" class="profile-dropdown" style="position: absolute; left: 0; top: 48px; width: 240px; background: white; border: 1px solid #E2E8F0; border-radius: 14px; box-shadow: 0 10px 30px rgba(0,0,0,0.08); padding: 12px; z-index: 100;">
          ...
          <c:forEach var="partner" items="${partnerList}">
              <c:choose>
                  <c:when test="${partner.partnerId == currentPartner.partnerId}">
                      <a href="#" class="dropdown-item" style="display:flex; align-items:center; gap:8px; padding:10px 12px; text-decoration:none; color:#1E293B; font-weight:800; font-size:13px; border-radius:8px; background:#F1F5F9; margin-bottom:4px;">
                        <span style="color:var(--primary);">✓</span> ${partner.partnerName}
                      </a>
                  </c:when>
                  <c:otherwise>
                      <a href="?tab=${activeTab}&partnerId=${partner.partnerId}" class="dropdown-item" style="display:flex; justify-content:space-between; align-items:center; padding:10px 12px 10px 28px; text-decoration:none; color:#64748B; font-weight:600; font-size:13px; border-radius:8px; transition:0.2s;">
                        <span>${partner.partnerName}</span>
                        <c:if test="${partner.status == 'PENDING'}">
                            <span style="font-size:10px; background:#F1F5F9; padding:2px 6px; border-radius:4px; font-weight:800; color:#94A3B8;">심사중</span>
                        </c:if>
                      </a>
                  </c:otherwise>
              </c:choose>
		  ...
		  <a href="${pageContext.request.contextPath}/partner/main?tab=new_apply" class="dropdown-item" style="display:block; padding:12px; text-decoration:none; color:var(--primary); font-weight:800; font-size:13px; border-radius:8px; text-align:center; background:var(--primary-10);">
	          ➕ 새로운 숙소 추가하기 (분점)
	      </a>
      </div>
    </div>
