...
<div class="modal-overlay" id="roomModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); backdrop-filter: blur(2px); z-index: 9999; display: none; align-items: center; justify-content: center;">
    <div class="modal-content" style="width: 600px; max-height: 90vh; overflow-y: auto; background: white; border-radius: 16px; padding: 24px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
        ...
        <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
            <h2 id="roomModalTitle" style="font-size: 18px; font-weight: bold; margin: 0;">🛏️ 새 객실 등록</h2>
            <button type="button" onclick="closeRoomModal()" style="background: none; border: none; font-size: 24px; cursor: pointer; color: #8B92A5;">✕</button>
        </div>
        ...
        <div class="modal-body">
            <form id="roomForm" onsubmit="event.preventDefault(); saveRoom();">
                ...
                <div style="margin-bottom: 16px;">
                    ...
                    <input type="text" id="roomName" class="form-control" placeholder="예: 오션뷰 스위트룸" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; outline:none;" required>
                </div>

                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px; margin-bottom: 16px;">
                    <div>
                        <label style="display:block; font-size:12px; font-weight:700; margin-bottom:6px;">기준 인원 (roombasecount)</label>
                        <input type="number" id="roombasecount" class="form-control" value="2" min="1" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; outline:none;" required>
                    </div>
                    <div>
                        <label style="display:block; font-size:12px; font-weight:700; margin-bottom:6px;">최대 인원 (max_capacity)</label>
                        <input type="number" id="maxCapacity" class="form-control" value="4" min="1" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; outline:none;" required>
                    </div>
                </div>
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px; margin-bottom: 16px;">
                    <div>
                        ...
                        <input type="number" id="roomCount" class="form-control" value="10" min="1" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; outline:none;" required>
                    </div>
                    <div>
                        <label style="display:block; font-size:12px; font-weight:700; margin-bottom:6px;">기본 요금 (amount)</label>
                        <input type="number" id="amount" class="form-control" placeholder="예: 150000" min="0" style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; outline:none;" required>
                    </div>
                </div>
                ...
                <div style="margin-bottom: 16px;">
                    <label style="display:block; font-size:12px; font-weight:700; margin-bottom:8px;">판매 상태 (is_active)</label>
                    <div style="display:flex; gap:20px; font-size:14px; padding: 10px; border: 1px solid var(--border); border-radius: 8px;">
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
                            <input type="radio" name="isActive" value="1" checked style="accent-color: var(--primary);"> 판매중
                        </label>
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
                            <input type="radio" name="isActive" value="0" style="accent-color: var(--primary);"> 판매중지
                        </label>
                    </div>
                </div>

                <div style="margin-bottom: 24px;">
                    <label style="display:block; font-size:12px; font-weight:700; margin-bottom:8px;">객실 시설 (facility)</label>
                    <div style="display:flex; flex-wrap:wrap; gap:12px; padding: 12px; background: #F8FAFC; border-radius: 8px; border: 1px solid #E2E8F0; font-size: 13px;">
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="checkbox" name="facility" value="roomaircondition" style="accent-color: var(--primary);"> ❄️ 에어컨</label>
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="checkbox" name="facility" value="roomtv" style="accent-color: var(--primary);"> 📺 TV</label>
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="checkbox" name="facility" value="roominternet" style="accent-color: var(--primary);"> 📶 와이파이</label>
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="checkbox" name="facility" value="roombath" style="accent-color: var(--primary);"> 🛁 욕조</label>
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer;"><input type="checkbox" name="facility" value="roomrefrigerator" style="accent-color: var(--primary);"> 🧊 냉장고</label>
                    </div>
                </div>
